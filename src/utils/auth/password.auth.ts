...
export class PasswordValidator {
  ...
  validate(password: string, username: string, email: string) {
    ...
    if (strength.score < this.minScore) {
      ...
      const suggestion =
        strength.feedback.suggestions[0] ?? 'Use a stronger password';
    ...
    const similarityEmail = this.calculateSimilarity(
      password,
      email.split('@')[0]
    );
    ...
    if (similarityUsername > this.maxSimilarity) {
      errors.push(
        `Password is too similar to username (${(similarityUsername * 100).toFixed(0)}%)`
      );
    }
    if (similarityEmail > this.maxSimilarity) {
      errors.push(
        `Password is too similar to email (${(similarityEmail * 100).toFixed(0)}%)`
      );
    }
